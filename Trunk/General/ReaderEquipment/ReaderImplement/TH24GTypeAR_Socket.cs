...
{
    public class TH24GTypeAR_Socket : AbstractReader
    {
        #region 变量定义
        ...
        public override List<TagInformationInfo> GetTagList(string strAntennaNum)
        {
            this._isReading = true;
            this._currentDataType = EnumRecDataMode.CollectRecordMode;
            OrderSend_GetBatchData(Convert.ToInt32(strAntennaNum));
            //this.Invoke(new DelegateDataReceiving(DataReceive));
            DataReceive();
            return null;
        }

        public override List<TagInformationInfo> GetTagList(string strAntennaNum, bool lIsDelHistory)
        {
